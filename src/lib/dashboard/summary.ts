...
async function loadSummary(locale: string): Promise<DashboardSummary> {
  ...
  const [
    ...
    prisma.futureOrder.count({
      where: { ...activeOrderWhere, orderCategory: ORDER_CATEGORY_WEDDING },
    }),
    prisma.futureOrder.count({
      where: { ...activeOrderWhere, orderCategory: ORDER_CATEGORY_DAILY },
    }),
    ...
    prisma.financialDocument.count({
      where: {
        documentType: "דוח Z",
        OR: [
          { docDate: { gte: today0, lte: todayEnd } },
          { docDate: null, createdAt: { gte: today0, lte: todayEnd } },
        ],
      },
    }),
  ]);
